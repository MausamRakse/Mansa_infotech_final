      ...
      >
        ...
        <div className="px-6 py-5 border-b border-border/40 flex items-center justify-between bg-muted/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[16px] font-black text-surface-foreground uppercase tracking-wider flex items-center gap-2">
                Select Voice
              </h2>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 flex flex-col bg-muted/5">
          
          {/* Browse by Language Container */}
          <div className="bg-surface border border-border/40 p-5 rounded-2xl shadow-sm mb-6 flex flex-col gap-4">
            <h3 className="font-bold text-[13px] text-surface-foreground uppercase tracking-wider">Browse by Language</h3>
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              <button 
                ...
                className={`px-4 py-1.5 rounded-lg text-[12px] font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 pt-2 border-t border-border/10 w-full">
              <div className="relative flex-1 w-full">
                <Search className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-textMuted pointer-events-none" />
                <input 
                  type="text" 
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Search voices..."
                  className="w-full pl-10 pr-4 py-2 text-[12px] bg-muted/20 border border-border rounded-xl text-surface-foreground placeholder:text-textMuted outline-none focus:border-primary transition-all w-full"
                />
              </div>
              ...
              <select
                ...
                className="px-4 py-2 rounded-xl bg-muted/20 border border-border text-[12px] font-bold text-surface-foreground outline-none focus:border-primary cursor-pointer w-full sm:w-auto"
              >
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-[16px] font-black text-surface-foreground uppercase tracking-tight">Available Voices</h3>
              <p className="text-[11px] text-textMuted">Organized by language for easy browsing</p>
            </div>
            <span className="text-[12px] font-black text-[#1e293b] bg-muted px-3 py-1 rounded-full border border-border/25">
          ...
          {filteredVoices.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-12 text-center gap-3 opacity-60">
              ...
              <p className="text-[12px] text-textMuted max-w-[300px]">Try adjusting your search queries or clearing the filter variables.</p>
            </div>
          ) : (
            <div className="bg-surface rounded-2xl border border-border/40 shadow-sm overflow-hidden overflow-x-auto">
              <table className="w-full text-left border-collapse text-[13px] min-w-[700px]">
                <thead>
                  <tr className="border-b border-border/30 text-textMuted font-bold text-[11px] uppercase tracking-widest bg-muted/10">
                    <th className="px-6 py-4">Voice Name</th>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex flex-col">
                            <span className="font-bold text-surface-foreground text-[14px]">{voice.name}</span>
                            {voice.badge && (
                              <span className="inline-flex mt-1 text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded bg-blue-500/10 text-blue-500 border border-blue-500/10 w-fit">
                                {voice.badge}
                              </span>
                            )}
                          </div>
                        ...
                        {/* Language Tag */}
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold bg-purple-500/10 text-purple-600 border border-purple-500/10">
                            <span>🇮🇳</span>
                            {voice.language}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="inline-flex items-center gap-1.5 text-textMuted font-medium">
                        ...
                        <td className="px-6 py-4 text-center whitespace-nowrap">
                          <button
                            ...
                            className={`px-4 py-1.5 rounded-lg flex items-center justify-center gap-1.5 text-[11px] font-black tracking-wide cursor-pointer transition-all active:scale-95 shadow-sm min-w-[85px] mx-auto ${
                              isPlaying
                        ...
                        <td className="px-6 py-4 text-right whitespace-nowrap">
                          <button
                            ...
                            className={`px-5 py-1.5 rounded-lg text-[12px] font-black cursor-pointer transition-all shadow-sm ${
                              isSelected
