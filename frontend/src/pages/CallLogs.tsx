...
const CallLogs = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto h-full flex flex-col animate-in fade-in duration-500">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-textPrimary tracking-tight">Call Logs</h1>
        <p className="text-textMuted mt-1">Review activity, playback recordings, and read transcripts from your agents.</p>
      </div>

      <div className="flex-1 bg-white rounded-[12px] border border-border card-shadow flex flex-col min-h-0 overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 flex flex-col items-center justify-center text-textMuted h-full">
            ...
            <p className="text-textPrimary font-bold text-[18px]">No call history yet</p>
              ...
              <tbody className="divide-y divide-border/60">
                {logs.map((log, i) => (
                  <tr key={i} className="hover:bg-surface/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      {log.recording_url ? (
                        <a href={log.recording_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-textPrimary hover:text-primary transition-colors text-[13px] font-medium">
