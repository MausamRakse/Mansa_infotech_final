...
const Settings = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto h-full flex flex-col animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-[12px] border border-border card-shadow">
        <div className="px-6 py-5 border-b border-border">
          <h2 className="text-[16px] font-bold text-textPrimary">Platform Settings</h2>
        </div>
        <form onSubmit={handleSave} className="p-6 flex flex-col gap-6">
          
          <div className="flex flex-col gap-2">
            <label className="text-[14px] font-semibold text-textPrimary">Platform Name</label>
            <p className="text-[12px] text-textMuted mb-1">Customize the display name in the sidebar.</p>
            <input 
              type="text"
              value={platformName}
              onChange={e => setPlatformName(e.target.value)}
              className="border border-border rounded-[8px] px-3 py-2 text-[14px] bg-white outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all w-full max-w-md"
            />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-md">
            <div className="flex flex-col gap-2">
              <label className="text-[14px] font-semibold text-textPrimary">Default Language</label>
              <select 
                value={defaultLanguage}
                onChange={e => setDefaultLanguage(e.target.value)}
                className="border border-border rounded-[8px] px-3 py-2 text-[14px] bg-white outline-none focus:border-primary w-full"
              >
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[14px] font-semibold text-textPrimary">Default Voice</label>
              <select 
                value={defaultVoice}
